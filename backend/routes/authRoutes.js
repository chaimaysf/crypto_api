...
if (process.env.JWT_REFRESH_SECRET === undefined) throw new Error("JWT_REFRESH_SECRET is not defined"); //
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const validationLink = `${process.env.FRONTEND_URL}/verify-email?token=${emailVerificationToken}`;
    await sendEmail(
      email,
...
router.post("/refresh", async (req, res) => {
  try {
    ...
    const tokenResult = await pool.query("SELECT * FROM refresh_token WHERE user_id = $1", [decoded.id]);
    ...
    await pool.query("DELETE FROM refresh_token WHERE token = $1", [storedToken.token]);
    ...
    await pool.query(
      "INSERT INTO refresh_token (token, user_id, expires_at) VALUES ($1, $2, $3)",
      [hashedNewRefreshToken, decoded.id, new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)]
    );
